...
0x003F | Failed to establish or maintain a connection to a network or remote peer.
0x004B | The remote hostname could not be resolved.
...
0x0069 | The http request failed due to an invalid certificate.
0x006D | The target device isn't in the network.
0x0073 | The string could not be converted to a UUID.
0x0085 | The PlayFab service detected a concurrent, conflicting change and could not complete the operation. Most commonly, this is caused by multiple clients calling the same PlayFab API with the same PlayFab entity..
0x0089 | Setting work mode after Party initialization is forbidden.
0x008D | The operation could not be performed in the current work mode.
0x008E | The operation could not be performed because it was not invoked from a thread in a COM multithreaded apartment.
0x008F | Text moderation service is not enabled for this title; please contact Microsoft to enable.
0x0090 | The service encountered an error performing text moderation.
0x0093 | Moderation of text chat on direct peer connections is unsupported.
0x0095 | Setting profiling callbacks after Party initialization is forbidden.
0x00A0 | The http request failed due to an error in the SSL certificate sent by the server.
...
0x3023 | The user and/or device are not authorized to use the requested sandbox or are not authorized to use the requested title in the current sandbox.
0x3024 | When using the Microsoft Game Core version of the library, it must be cleaned up when the title is suspended and re-initialized when the title is resumed.
